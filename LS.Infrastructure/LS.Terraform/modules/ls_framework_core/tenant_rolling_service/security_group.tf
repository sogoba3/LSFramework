...
resource "aws_security_group" "ls_framework_tenant_ecs_service_sg" {
...
resource "aws_vpc_security_group_ingress_rule" "gateway_service_ingress" {
  security_group_id = aws_security_group.ls_framework_tenant_ecs_service_sg.id
...
resource "aws_vpc_security_group_ingress_rule" "internal_services_ingress" {
  for_each = var.ls_framework_ecs_service_sg_ids
